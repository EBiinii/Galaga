...
public class Keyplay1 extends ImageIcon {
	...
	public Keyplay1(URL imgURL, int x, int y, int margin, int steps, int xBoundary, int yBoundary) {
		// imgPath : 그림 파일의 경로명
		// x, y : 이미지의 시작 위치 좌표
	...
	public Keyplay1(URL imgURL, int margin, int steps, int xBoundary, int yBoundary) {
		this (imgURL, 0, 0, margin, steps, xBoundary, yBoundary);
		x= (int) (Math.random() * 530)+50;
		y= (int) (Math.random() * 400);//캐릭터와 만나지 않도록 설정
	}
	
	public Keyplay1(URL imgURL, int margin, int xBoundary, int yBoundary) {
