 ...
 static String name1,name2;
    			...
    			while (true) {
    				...
    				// 읽은 메시지의 종류에 따라 각각 할일이 정해져 있음
           	 		...
           	 		if (type == GameMessage.MsgType.LOGIN) {		  // 클라이언트 로그인 요청
           	 			...
           	 			System.out.println("내 점수 "+message.count+"점");
           	 			System.out.println("보낸 사람  "+message.getSender());
           	 		//cast(message.getSender(),message.count);
           	 		...
           	 		else if(type == GameMessage.MsgType.GAME_ITEM){
           	 			broadcastMessage(new GameMessage(GameMessage.MsgType.GAME_ITEM, message.userCheck));
           	 		}
           	 		else {
           	 			// 정체가 확인되지 않는 이상한 메시지?
    ...
    private synchronized void handleLogin(String user, ObjectOutputStream writer) {
	   try {
		   ...
		   if(numcnt>1){
	            writer.writeObject(new GameMessage(GameMessage.MsgType.LOGIN_FULL,"","","사용자 가득 찼음"));
	            return;
	         }
	   } catch (Exception ex) {
	...
	public void cast(String sender,int count){
	      Set<String> s = clientOutputStreams.keySet();   // 먼저 등록된 사용자들을 추출하고 하나하나에 메시지 보냄
	      // 그러기 위해서 먼저 사용자 리스트만 추출
	      Iterator<String> it = s.iterator();
	      String user;
	      ...
	      System.out.println("cast send "+ sender);
	      

	      System.out.println("cast count"+ count);
	      ...
	      while(it.hasNext()) {
	         ...
	         if(user.equals(sender)){
	        	 System.out.println("= cast re "+ user);
	            continue;
	         }
	         else{
	            try {
	               ...
	               writer.writeObject(new GameMessage(GameMessage.MsgType.GAME_P,count));
	               
	               //writer.writeObject(new GameMessage(GameMessage.MsgType.GAME_ITEM,count)); 
	              
	               System.out.println("클라이언트로 전송했다.");
	            }
	            catch(Exception ex) {
	               System.out.println("cast 이상 발생");
	               ex.printStackTrace();
	            }
	            return;
	         }
